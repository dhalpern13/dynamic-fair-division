...
def ranking_alg(num_agents, dataset, compare_to_all=False, sampling_length=lambda epoch: epoch ** 4,
                exploiting_length=lambda epoch: epoch ** 8, total_items=10000):
    dataset_iter = dataset_sampler(dataset, num_agents, total_items)
    items = []
    try:
        for epoch in count(1):
            samples = [SortedList() for _ in range(num_agents)]
            for _ in range(sampling_length(epoch)):
                for i in range(num_agents):
                    new_item = next(dataset_iter)
                    samples[i].add(new_item[i])
                    items.append((i, new_item))
            for _ in range(exploiting_length(epoch)):
                new_item = next(dataset_iter)
...
def ranking_alg_with_sample_epochs(num_agents, dataset, sample_prob_func, delete_prob_func, total_items=10000):
    items = []
    samples = [SortedList() for _ in range(num_agents)]
...
def deterministic_squareroot(num_agents, dataset, total_items=10000):
    items = []
    samples = [SortedList() for _ in range(num_agents)]

    #TODO inital 20 items
...
def compare_to_everything(num_agents, dataset, total_items=10000, starting_sample=20):
    items = []
    dataset_iter = dataset_sampler(dataset, num_agents, total_items)
    samples = [SortedList() for _ in range(num_agents)]
...
def welfare_maximizing(num_agents, dataset, total_items=10000):
    items = []
...
def quantile_maximizing(num_agents, dataset, total_items=10000):
